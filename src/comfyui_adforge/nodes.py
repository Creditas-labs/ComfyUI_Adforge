...
class Example:
    ...
    FUNCTION (`str`):
        The name of the entry-point method. For example, if `FUNCTION = "execute"`
        then it will run Example().execute()
    OUTPUT_NODE ([`bool`]):
    ...
    execute(s) -> tuple || None:
        The entry point method. The name of this method must be the same
        as the value of property `FUNCTION`.
        For example, if `FUNCTION = "execute"` then this method's name must be `execute`,
        if `FUNCTION = "foo"` then it must be `foo`.
    """
    ...
    def INPUT_TYPES(s):
        ...
        Some types (string): "MODEL", "VAE", "CLIP", "CONDITIONING", "LATENT",
        "IMAGE", "INT", "STRING", "FLOAT".
        ...
        Returns: `dict`:
            ...
            - Value input_fields (`dict`): Contains input fields config:
                ...
                * Value field_config (`tuple`):
                    + First value is a string indicate the type of field or a list
                      for selection.
            ...
            "required": {
                ...
                "float_field": (
                    "FLOAT",
                    {
                        ...
                        "max": 10.0,
                        "step": 0.01,
                        "round": 0.001,  # The value represeting the precision to round to,
